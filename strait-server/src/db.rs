...
fn apply_migrations(conn: &Connection) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !columns
        .iter()
        .any(|name| name == "dispatch_idempotency_key")
    {
    ...
    let mut stmt = tx.prepare("PRAGMA table_info(pipeline_runs)")?;
    let columns = stmt
        .query_map([], |row| row.get::<_, String>(1))?
        .collect::<Result<Vec<_>, _>>()?;
    drop(stmt);
    if !columns.iter().any(|name| name == "cancel_requested_at") {
        tx.execute_batch("ALTER TABLE pipeline_runs ADD COLUMN cancel_requested_at TEXT;")?;
    }
    ...
    drop(stmt);
    if !columns.iter().any(|name| name == "cancel_requested_at") {
        tx.execute_batch("ALTER TABLE job_runs ADD COLUMN cancel_requested_at TEXT;")?;
    }
    if !columns.iter().any(|name| name == "cancel_started_at") {
    ...
    if !columns.iter().any(|name| name == "cancel_retry_count") {
        tx.execute_batch(
            "ALTER TABLE job_runs ADD COLUMN cancel_retry_count INTEGER NOT NULL DEFAULT 0;",
        )?;
    }
    ...
    if !columns.iter().any(|name| name == "infra_retry_count") {
        tx.execute_batch(
            "ALTER TABLE job_runs ADD COLUMN infra_retry_count INTEGER NOT NULL DEFAULT 0;",
        )?;
    }
    if !columns.iter().any(|name| name == "last_infra_retry_at") {
        tx.execute_batch("ALTER TABLE job_runs ADD COLUMN last_infra_retry_at TEXT;")?;
    }
