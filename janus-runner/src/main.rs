...
use config::Config;
use janus_lib::{RunnerCapabilitiesResponse, RunnerRouteTemplate};
use jobs::JobStore;
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "janus_runner=info,axum=info".into()),
        )
        .json()
        .flatten_event(true)
        .init();
...
fn config_path() -> String {
    env::args()
        .nth(1)
        .or_else(|| env::var("JANUS_RUNNER_CONFIG").ok())
...
mod tests {
    ...
    use janus_lib::{RUNNER_PROTOCOL_VERSION, RunnerRoute};
    ...
    async fn capabilities_reports_runner_protocol_versions() {
        ...
        let response = app
            .oneshot(
                Request::get(RunnerRoute::Capabilities.path())
                    .header("x-janus-key-id", "jobs-read")
                    .body(Body::empty())
                    .expect("request should build"),
            )
            .await
        ...
        let body = axum::body::to_bytes(response.into_body(), usize::MAX)
            .await
            .expect("response body");
        let payload: janus_lib::RunnerCapabilitiesResponse =
    ...
    async fn protected_routes_enforce_permissions() {
        ...
        let cases = vec![
            (
                Request::get("/jobs")
                    .header("x-janus-key-id", "jobs-run")
                    .body(Body::empty())
                    .expect("request should build"),
            ...
            (
                Request::get("/runs/job_missing")
                    .header("x-janus-key-id", "logs-read")
                    .body(Body::empty())
                    .expect("request should build"),
                StatusCode::FORBIDDEN,
            ),
            ...
            (
                Request::post("/artifacts")
                    .header("x-janus-key-id", "artifacts-read")
                    .body(Body::from("body"))
                    .expect("request should build"),
                StatusCode::FORBIDDEN,
            ),
            ...
            (
                Request::get("/jobs")
                    .header("x-janus-key-id", "wrong-key")
                    .body(Body::empty())
                    .expect("request should build"),
...
"#,
    ...
    fn temp_dir(label: &str) -> PathBuf {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let path = std::env::temp_dir().join(format!("janus-runner-main-{label}-{unique}"));
