...
`strait-lib` contains shared wire and schema types used by `strait-server` and
`strait-runner`.
...
When in doubt, keep `strait-lib` limited to types that are serialized,
deserialized, or compared by both `strait-server` and `strait-runner`.
