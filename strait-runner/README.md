...
Start `strait-server` once and copy its runner trust key from the Runners page, or from the configured `runner_auth.public_key_path`, into `[[auth.servers]]`.
...
```http
X-Strait-Key-Id: <key-id>
X-Strait-Timestamp: <rfc3339 timestamp>
X-Strait-Nonce: <unique nonce>
X-Strait-Content-SHA256: <hex sha256 of body>
X-Strait-Signature: ed25519:<base64 signature>
```
...
Direct protected API calls also need the `X-Strait-*` signed request headers normally generated by `strait-server`.
