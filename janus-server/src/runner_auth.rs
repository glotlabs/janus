...
fn generate_key_id() -> String {
    ...
    format!("janus-{}-{}", now.format("%Y%m"), hex::encode(random))
}
...
mod tests {
    ...
    fn rotation_updates_config_and_keeps_existing_public_key_file() {
        ...
        assert!(rotated.runner_auth.key_id.starts_with("janus-"));
        assert!(
            rotated
                .runner_auth
                .private_key_path
    ...
    fn init_adds_generated_runner_auth_to_minimal_config() {
        ...
        let config = Config::load_from_path(&config_path).expect("config should load");
        assert!(config.runner_auth.key_id.starts_with("janus-"));
...
"#,
    ...
    fn temp_dir(label: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("janus-server-{label}-{}", Uuid::now_v7()));
