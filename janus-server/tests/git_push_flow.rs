...
fn write_config(temp: &Path, port: u16, runner_base_url: &str) -> PathBuf {
    ...
    let control_socket = control_socket_dir.join(format!("janus-{port}.sock"));
...
impl ServerProcess {
    fn spawn(config_path: &Path) -> Self {
        let child = Command::new(env!("CARGO_BIN_EXE_janus-server"))
            .args(["serve", "--config"])
            .arg(config_path)
            .stdout(Stdio::null())
            .stderr(Stdio::null())
...
fn bootstrap_admin(config_path: &Path) {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_janus-server"))
        .args([
            "admin",
            "bootstrap-admin",
            "--username",
